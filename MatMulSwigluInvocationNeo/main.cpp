...
#include "acl/acl.h"
#include "aclrtlaunch_matmul_swiglu_custom.h"
#else
#include "tikicpulib.h"
extern "C" void matmul_swiglu_custom(uint8_t *a, uint8_t *b, uint8_t *c, uint8_t *workspace, uint8_t *tiling);
#endif
extern uint8_t *GenerateTiling();

int32_t main(int32_t argc, char *argv[])
{
    int16_t M = 128, K = 2048, N = 12288;
    auto ascendcPlatform = platform_ascendc::PlatformAscendCManager::GetInstance();
    size_t aFileSize = M * K * sizeof(uint16_t);  // uint16_t represent half
    size_t bFileSize = 2 * N * K * sizeof(uint16_t); // uint16_t represent half
    size_t cFileSize = M * N * sizeof(uint16_t);
    size_t tilingFileSize = sizeof(TCubeTiling);
    size_t userWorkspaceSize = 2 * M * N * sizeof(uint16_t);
...
// cpu域debug
    ...
    ICPU_RUN_KF(matmul_swiglu_custom, blockDim, a, b, c, workspace, tiling);
...
// npu域
    ...
    ACLRT_LAUNCH_KERNEL(matmul_swiglu_custom)
